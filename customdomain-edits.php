...
function custom_domain_text () {
    ...
    # Note: In order to set a Custom Domain Name you must have purchased and pointed your custom domain name already. Do not use this for changing a sub domain name! To do that contact info@bizlaunchuniversity.com and request a sub domain name change.
    ...
    if ($custom_domain) {    
      ...
      if ($main_domain[0]['ip'] != $server_addr) {
          echo "<div class=\"custom-domain-warning\">ERROR: $custom_domain is not pointed to $server_addr, please check your DNS!</div>";
      }
      if ($main_domain[0]['ip'] == $server_addr) {
          echo "<div class=\"custom-domain-success\">SUCCESS: $custom_domain is pointed properly!</div>";
      }
      if ($www_domain[0]['ip'] != $server_addr) {
          echo "<div class=\"custom-domain-warning\">ERROR: www.$custom_domain is not pointed to $server_addr</div>";
      }
      if ($www_domain[0]['ip'] == $server_addr) {
      ...
      echo "<br><br>Do not use this for changing a sub domain name! To do that contact info@bizlaunchuniversity.com and request a sub domain name change.";
      echo "<br><br>For more information on to use this feature visit.</div>";
      echo "<div class=\"custom-domain-link\"><a href=\"https://support.bluinf.com\" target=\"_blank\">Bizlaunch University Support</a></div>";
